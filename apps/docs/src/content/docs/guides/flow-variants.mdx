...
## Providing components for all variants

When using flow variants, you must **provide components for all steps** across all variants. Components use the generic `useFlow()` hook to work with any flow:
...
:::tip[TypeScript Enforcement]
**TypeScript requires all steps from all variants to be provided.** If you forget a step that exists in any variant, you'll get a compile-time error. This ensures your app won't crash when users switch between variants.
:::
...
- [Branching](/guides/branching-flows) - Conditional navigation within flows
