...
```typescript
{
  ...
  start: string;                 // Default starting step name
...
`start` is the default first step for fresh flow state. If your app needs to enter the flow at a different step, pass [`initialStepId`](/api-reference/flow-component#initialstepid) to `<Flow />`.
