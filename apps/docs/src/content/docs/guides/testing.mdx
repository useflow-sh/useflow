...
it("should persist context across steps", () => {
  ...
  function Step2() {
    const { context } = useFlow<{ email: string }>();
...
### Testing context-driven branching
...
### Testing component-driven branching
...
it("should handle form input and navigation", async () => {
  ...
  function CompleteStep() {
    const { context } = useFlow<{ name: string }>();
...
- [TypeScript](/getting-started/typescript) - Type-safe testing
- [Callbacks](/guides/callbacks) - Test flow callbacks
- [Persistence](/guides/persistence) - Test persistence
- [Branching](/guides/branching) - Test branching logic
