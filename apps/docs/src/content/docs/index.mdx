...
import { Card, CardGrid, Steps, Code, Tabs, TabItem } from "@astrojs/starlight/components";

## Build multi-step flows without the complexity

<CardGrid stagger>
  <Card title="Declarative" icon="document">
    Define your flow structure once. No more `if/else` chains scattered across components.
  </Card>
  <Card title="Built-in Persistence" icon="seti:db">
    Enable with 2 lines of code. Sers continue exactly where they left off.
  </Card>
...
2. ### Create your step components
   ...
   function UserTypeStep() {
     ...
     const handleSubmit = () => {
       next(); // Automatically routes based on accountType
     };
...
**That's it!** Users can now close their browser and return exactly where they left off. No manual state management, no localStorage code, no routing logic.
...
- ⚙️ **Configuration wizards** - Complex setup broken into steps
- 📋 **Application processes** - Job applications, loan applications
...
<CardGrid>
  <Card title="Remote Configuration" icon="setting">
    Modify flows without deploying. Pdate step order and branching logic from your backend.
  </Card>
...
<CardGrid>
  <Card title="Quick Start" icon="rocket">
    [Build your first flow →](/getting-started/quick-start)
    
    Ship a working flow in minutes
  </Card>
  <Card title="Why useFlow?" icon="information">
    [Learn more →](/getting-started/why-useflow)
    
    Benefits and comparisons
  </Card>
  <Card title="Examples" icon="open-book">
    [Browse recipes →](/recipes/onboarding)
    
    Production-ready patterns
  </Card>
  <Card title="API Reference" icon="document">
    [View docs →](/api-reference/define-flow)
