...
<Aside type="tip">
  **Why `.with<Context>()`?** You get automatic context typing in every component that uses `myFlow.useFlowState()` - no need to specify the generic on every hook call. Plus, you get step-specific navigation types.
</Aside>

### Alternative: Generic useFlowState hook
...
### 3. Use typed step IDs for custom entry points

`initialStepId` is checked against the flow's known step IDs:

```tsx
<Flow
  flow={myFlow}
  initialStepId="profile" // ✅ Must be a real step in myFlow
  initialContext={{ email: "" }}
/>

<Flow
  flow={myFlow}
  initialStepId="missingStep" // ❌ TypeScript error
  initialContext={{ email: "" }}
/>
```
