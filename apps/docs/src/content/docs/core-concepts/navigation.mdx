...
**Navigation** is how users move through your flow. useFlow provides several navigation methods that are type-safe, flexible, and work seamlessly with your flow definition.
...
```tsx
function MyStep() {
  const { next, back, path } = useFlow();
  
  // Check if we can go back by looking at path length
...
function Step1() {
  const { next } = flow.useFlow({ step: 'step1' });
...
function ChoiceStep() {
  const { next } = flow.useFlow({ step: 'choice' });
...
function PreferencesStep() {
  const { next, skip, context } = useFlow();
...
You can check if back navigation is available by examining the path:

```tsx
function MyStep() {
  const { back, path } = useFlow();
  
  // Can go back if we're not on the first step
  const canGoBack = path.length > 1;
...
- Path has more than 1 entry (not on first step)
...
function MyStep() {
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      if (e.key === 'Enter') {
        next();
      } else if (e.key === 'Escape' && path.length > 1) {
        back();
      }
    };
...
- [**Branching Guide**](/guides/branching) - Implement conditional navigation
