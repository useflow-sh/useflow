...
Context-driven branching uses a **resolve function** to automatically determine the next step based on the current context. The flow decides where to go without the component needing to know about routing logic.
...
- **Separation of concerns** - keep routing logic out of UI components
...
The component just calls `next()` with no arguments - the resolve function handles routing:
...
Component-driven branching puts routing control in the component's hands. The component explicitly chooses which step to navigate to by passing a target to `next()`.

### When to use

- **User makes explicit navigation choice** (skip vs continue, save draft vs publish)
- **UI-driven routing** where the button clicked determines the path
...
|---------|----------|---------|
| **Context-Driven** | Business logic, automatic decisions | User role routing, feature flags |
...
Leverage TypeScript to catch routing errors at compile time:
...
export const onboardingFlow = defineFlow({
  ...
  steps: {
    ...
    // Context-driven: automatic routing based on userType
    userType: {
      next: ["businessDetails", "setupPreference"]
    },
    businessDetails: {
