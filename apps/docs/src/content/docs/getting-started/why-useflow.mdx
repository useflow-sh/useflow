...
- ❌ **Lost user progress** - No built-in persistence means angry users
- ❌ **No analytics visibility** - Can't see where users drop off
- ❌ **Debugging nightmares** - No history of what happened
- ❌ **Prop drilling hell** - Passing callbacks through every component
- ❌ **Type safety? Forget it** - Strings everywhere, runtime errors
- ❌ **Testing complexity** - Mocking all that state logic
- ❌ **New developer confusion** - "Where does this flow go?"
...
function App() {
  return (
    <Flow
      flow={onboardingFlow}
      persister={createPersister({ store: createLocalStorageStore() })} // ✅ Persistence in 1 line!
      onNext={({ from, to }) => {              // ✅ Analytics in 1 line!
...
- 🔴 Reinvent navigation, history, branching
