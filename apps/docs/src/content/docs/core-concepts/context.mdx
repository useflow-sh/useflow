...
Use the `useFlow()` hook to access context:
...
<Flow
  ...
  persister={{
    store,
    saveMode: "auto", // Auto-save on every step change
  }}
>
  {({ renderStep }) =>
    renderStep({
...
// 3. Step components
function ProfileStep() {
  const { next, context } = onboardingFlow.useFlow({ step: "profile" });
...
function UserTypeStep() {
  const { next } = onboardingFlow.useFlow({ step: "userType" });
...
function BusinessStep() {
  const { next } = onboardingFlow.useFlow({ step: "business" });
...
- [**Branching Guide**](/guides/branching) - Use context for conditional navigation
