...
### Persistence with `initialStepId`

`initialStepId` only controls the step used when creating fresh flow state. When a valid saved state exists, restore wins:
...
If the user has saved progress at `complete`, the flow restores `complete`; it does not restart at `profile`. Calling `reset()` clears the saved state for this flow key and starts again from `initialStepId`.
