...
A step component is just a regular React component that uses the `useFlow()` hook to access flow state and navigation:
...
The `useFlow()` hook gives you access to:
...
```tsx
function MyStep() {
  const { isRestoring, next } = useFlow();
...
Use the generic `useFlow()` for more flexibility:

```tsx
function ProfileStep() {
  // Less type safety, but more flexible
  const { next, context } = useFlow<{ name: string }>();
...
- [**Branching Guide**](/guides/branching) - Implement conditional logic
