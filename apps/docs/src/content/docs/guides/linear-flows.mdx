...
function ProfileStep() {
  const { next, back, context } = onboardingFlow.useFlow({ step: "profile" });
...
function PreferencesStep() {
  const { next, back } = onboardingFlow.useFlow({ step: "preferences" });
...
function CompleteStep() {
  const { context, reset } = onboardingFlow.useFlow({ step: "complete" });
...
```tsx
function ProfileStep() {
  const { next } = onboardingFlow.useFlow({ step: "profile" });
...
```tsx
function PreferencesStep() {
  const { next, skip } = onboardingFlow.useFlow({ step: "preferences" });
...
```tsx
function EmailStep() {
  const { next } = onboardingFlow.useFlow({ step: "email" });
  const [email, setEmail] = useState("");
...
```tsx
function MyStep() {
  const { next, back, canGoBack } = useFlow();
...
function PreferencesStep() {
  const { next, back, skip } = onboardingFlow.useFlow({ step: "preferences" });
  const [theme, setTheme] = useState<"light" | "dark">("light");
  const [notifications, setNotifications] = useState(false);
...
- [**Branching Guide**](/guides/branching) - Add conditional logic
